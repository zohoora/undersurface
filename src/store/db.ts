...
export async function exportAllData() {
  const [entries, parts, memories, thoughts, interactions] = await Promise.all([
    db.entries.toArray(),
    db.parts.toArray(),
    db.memories.toArray(),
    db.thoughts.toArray(),
  ...
  const data = { entries, parts, memories, thoughts, interactions, exportedAt: new Date().toISOString() }
